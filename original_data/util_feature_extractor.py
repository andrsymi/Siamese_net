...
def detect_onsets(s, detect_params, filt_params):
    ...
    for ni in sample_idxs:
        ...
        if frame_enrg[num_idx] > detect_params['power_thresh']:  # and frame_enrg_ratio>detect_parameters['dBrat']: #activationEnergy>frame_enrg_smooth:
...
def extract_features(s, detect_params, filt_params, fft_length):
    ...
    for a in a_idxs:
        n = a[0]
        # if stamps[0] < n < stamps[3] and np.sum(active_idxs[n - detect_parameters['lookBackFrames']:n]) == 0:  # 3
